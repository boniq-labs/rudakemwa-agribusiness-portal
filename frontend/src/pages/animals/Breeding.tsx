    ...
    { key: 'notes', label: 'Notes', render: (item: any) => item.notes || '-' },
    {
      ...
      render: (item: any) => (
        <div className="actions">
          ...
          <button className="btn btn-sm" title="Delete" onClick={async (e) => { e.stopPropagation(); if (await confirm('Delete this breeding record?')) deleteMutation.mutate(item.id); }} disabled={deleteMutation.isPending}><Trash2 size={14} /></button>
        </div>
      ),
    },
  ];
