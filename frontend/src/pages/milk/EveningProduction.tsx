...
export default function EveningProduction() {
  ...
  return (
    <ModulePage
      ...
      subtitle="Record evening milk production"
      actions={<button className="btn btn-primary" onClick={() => { setEditingId(null); setForm(initialForm); setShowModal(true); }}><Plus size={16} /> Record Evening</button>}
    >
      <div className="stats-grid">
        <StatsCard title="Total Litres" value={`${totalLiters.toFixed(1)} L`} icon={Moon} color="#6366f1" />
        <StatsCard title="Records" value={collections.length} icon={Moon} color="#6366f1" />
      </div>
      <DataTable columns={columns} data={collections} loading={isLoading} emptyMessage="No evening records" />
      ...
      {showModal && (
        <div className="modal-overlay" onClick={() => setShowModal(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ background: 'var(--card-bg)', borderRadius: 'var(--radius)', padding: 24, width: '100%', maxWidth: 500 }}>
            <h3 style={{ marginBottom: 20 }}>{editingId ? 'Edit Evening Production' : 'Record Evening Production'}</h3>
