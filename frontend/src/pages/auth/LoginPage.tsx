...
import {
  AlertCircle, Eye, EyeOff, Leaf, Loader2, Lock, ShieldCheck,
...
export default function LoginPage() {
  ...
  const inputBase = (invalid: boolean) =>
    `w-full h-[50px] rounded-xl border-2 text-sm font-medium shadow-sm transition-all duration-200 focus:outline-none ${
      darkMode
        ? 'bg-gray-800/50 text-gray-100 border-gray-700 hover:border-gray-600 focus:bg-gray-800/80'
        : 'bg-white text-gray-900 border-gray-200 hover:border-gray-300 focus:bg-white'
    } ${
      invalid
        ? 'border-red-300 focus:border-red-400 focus:shadow-[0_0_0_4px_rgba(239,68,68,0.12)]'
        : darkMode
          ? 'focus:border-[#16a34a] focus:shadow-[0_0_0_4px_rgba(22,163,74,0.08)]'
          : 'focus:border-[#16a34a] focus:shadow-[0_0_0_4px_rgba(22,163,74,0.12)]'
    }`;
  ...
  return (
    <div className={`min-h-screen w-full flex items-center justify-center p-4 sm:p-6 lg:p-8 transition-colors duration-300 ${darkMode ? 'bg-[#0a0f0a]' : 'bg-[#f8fafc]'}`}>
      ...
      <div className="login-gradient-border w-full max-w-[1280px]" style={{ boxShadow: '0 25px 80px rgba(0,0,0,0.08), 0 10px 30px rgba(0,0,0,0.04)' }}>
        {/* Main Card */}
        <motion.div
          initial={{ opacity: 0, scale: 0.96, y: 20 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          transition={{ duration: 0.5, ease: [0.2, 0.9, 0.3, 1] }}
          className="w-full min-h-[640px] h-auto lg:h-[90vh] lg:max-h-[820px] rounded-2xl lg:rounded-[24px] overflow-hidden flex flex-col lg:flex-row bg-white"
        >
        {/* ============ LEFT PANEL ============ */}
        <div className="relative w-full lg:w-[40%] flex flex-col items-center justify-center lg:justify-between p-6 sm:p-8 lg:p-12 overflow-hidden"
          style={{
            background: 'radial-gradient(ellipse at 50% 30%, #12331f 0%, #07140c 55%, #030906 100%)',
          }}
        >
          {/* Farm background image with soft overlay for readability */}
          <div className="absolute inset-0">
            ...
            <div className="absolute inset-0"
              style={{
                background: 'linear-gradient(180deg, rgba(15,45,30,0.2) 0%, rgba(7,20,12,0.45) 45%, rgba(3,9,6,0.92) 100%)',
              }} />
          </div>
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            >
              ...
              <div
                className="relative w-24 h-24 sm:w-40 sm:h-40 lg:w-44 lg:h-44 rounded-full flex items-center justify-center overflow-hidden"
                style={{
            ...
            >
              <h1 className="text-2xl sm:text-[34px] font-extrabold text-white tracking-tight leading-none drop-shadow-2xl">
                {systemName}
              </h1>
              <p className="text-green-400/70 font-semibold text-[10px] sm:text-sm mt-2 sm:mt-3 tracking-[0.15em] uppercase">
                {farmName}
              </p>

              {/* Decorative divider */}
              <div className="flex items-center justify-center gap-2 mt-3 sm:mt-5 mb-2 sm:mb-3">
              ...
              <p className="hidden sm:block text-green-50/50 text-sm font-light italic leading-relaxed">
                Smart Farming, Better Future
              </p>
            </motion.div>
          </div>

          {/* Bottom Farm Illustrations (desktop only) */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ duration: 0.8, delay: 0.6 }}
            className="relative z-10 w-full hidden lg:flex items-center justify-center gap-6 sm:gap-8"
          >
            <motion.div
              animate={{ y: [0, -3, 0] }}
        ...
        {/* ============ RIGHT PANEL ============ */}
        <div className={`relative w-full lg:w-[60%] flex flex-col items-center justify-center p-5 sm:p-8 lg:p-12 overflow-y-auto transition-colors duration-300 ${darkMode ? 'bg-[#111811]' : 'bg-white'}`}>
          {/* Dark Mode Toggle */}
          ...
          <div className="w-full max-w-[400px]">
            ...
            >
              <h2 className={`text-[28px] sm:text-[36px] font-extrabold tracking-[-0.025em] leading-[1.1] transition-colors duration-300 ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                Welcome Back!
              </h2>
              ...
              <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-[18px]">
                <AnimatePresence>
                  {serverError && (
                    <motion.div
                      ...
                      className="bg-red-50/90 backdrop-blur-sm text-red-600 px-4 py-3 rounded-xl text-sm border border-red-200/60 font-medium overflow-hidden flex items-start gap-2.5"
                    >
                ...
                <div className="space-y-1.5">
                  <label
                    ...
                    className={`block text-sm font-semibold transition-colors duration-300 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}
                  >
                    Username or Email
                  </label>
                  <div className="relative group">
                    ...
                    {errors.username && (
                      <div className="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none">
                        <AlertCircle size={15} strokeWidth={2.2} className="text-red-400" />
                      </div>
                    )}
                  </div>
                  {errors.username && (
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative group">
                    ...
                    <button
                      ...
                      className={`absolute inset-y-0 right-0 pr-3.5 pl-3 flex items-center transition-all duration-200 hover:scale-105 ${darkMode ? 'text-gray-500 hover:text-gray-300' : 'text-gray-400 hover:text-gray-600'}`}
                    >
                ...
                <div className="pt-2">
                  <motion.button
                    ...
                    whileTap={isSubmitting ? undefined : { scale: 0.98, y: 0 }}
                    type="submit"
            ...
            <motion.div
              ...
              className="mt-9 sm:mt-10 pt-6 border-t border-gray-100/80"
            >
