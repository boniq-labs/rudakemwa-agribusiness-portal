...
export const createFuelRecord = async (req, res) => {
    try {
        ...
        const [result] = await pool.query('INSERT INTO fuel_records (vehicle_id, quantity, cost, date, fuel_type, mileage, notes) VALUES (?,?,?,?,?,?,?)', [vehicle_id, quantity, cost, date || new Date().toISOString().split('T')[0], fuel_type || null, mileage || null, notes || null]);
...
export const updateFuelRecord = async (req, res) => {
    try {
        const { vehicle_id, quantity, cost, date, fuel_type, mileage, notes } = req.body;
        const [old] = await pool.query('SELECT * FROM fuel_records WHERE id = ?', [req.params.id]);
        if (old.length === 0)
            return error(res, 'Fuel record not found', 404);
        await pool.query('UPDATE fuel_records SET vehicle_id=?, quantity=?, cost=?, date=?, fuel_type=?, mileage=?, notes=? WHERE id=?', [vehicle_id, quantity, cost, date, fuel_type, mileage, notes, req.params.id]);
