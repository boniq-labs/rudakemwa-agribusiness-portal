...
export const getMaintenanceRecords = async (req, res) => {
    try {
        ...
        if (req.query.vehicle_id) {
            filters += ' AND mr.vehicle_id = ?';
            params.push(req.query.vehicle_id);
        }
        if (req.query.maintenance_type) {
            filters += ' AND mr.maintenance_type = ?';
            params.push(req.query.maintenance_type);
        }
...
export const createMaintenanceRecord = async (req, res) => {
    try {
        ...
        const [result] = await pool.query('INSERT INTO vehicle_maintenance (vehicle_id, date, maintenance_type, description, cost, next_service_date) VALUES (?,?,?,?,?,?)', [vehicle_id, date, maintenance_type, description, cost, next_service_date]);
...
export const getDueMaintenance = async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT mr.*, v.vehicle_name as vehicle_name FROM vehicle_maintenance mr JOIN vehicles v ON mr.vehicle_id = v.id WHERE mr.next_service_date IS NOT NULL AND mr.next_service_date <= DATE_ADD(CURDATE(), INTERVAL 7 DAY) ORDER BY mr.next_service_date');
