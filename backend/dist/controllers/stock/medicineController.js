...
export const createMedicine = async (req, res) => {
    try {
        ...
        const [result] = await pool.query('INSERT INTO medicine_items (name, category, quantity, unit_price, expiry_date, supplier_id, reorder_level) VALUES (?,?,?,?,?,?,?)', [name, category, quantity, unit_price, expiry_date, supplier_id, reorder_level]);
...
export const updateMedicine = async (req, res) => {
    try {
        const { name, category, quantity, unit_price, expiry_date, supplier_id, reorder_level } = req.body;
        const [old] = await pool.query('SELECT * FROM medicine_items WHERE id = ?', [req.params.id]);
        if (old.length === 0)
            return error(res, 'Medicine not found', 404);
        await pool.query('UPDATE medicine_items SET name=?, category=?, quantity=?, unit_price=?, expiry_date=?, supplier_id=?, reorder_level=? WHERE id=?', [name, category, quantity, unit_price, expiry_date, supplier_id, reorder_level, req.params.id]);
...
export const getExpiringMedicines = async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT * FROM medicine_items WHERE deleted_at IS NULL AND expiry_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 30 DAY) ORDER BY expiry_date');
...
export const getExpiredMedicines = async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT * FROM medicine_items WHERE deleted_at IS NULL AND expiry_date < CURDATE() ORDER BY expiry_date');
        return success(res, rows);
    }
    catch (err) {
        return error(res, err.message);
