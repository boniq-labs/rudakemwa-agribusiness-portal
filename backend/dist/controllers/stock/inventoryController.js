...
export const createInventoryCategory = async (req, res) => {
    try {
        ...
        const [result] = await pool.query('INSERT INTO inventory_categories (name, description) VALUES (?,?)', [name, description]);
...
export const updateInventoryCategory = async (req, res) => {
    try {
        const { name, description } = req.body;
        const [old] = await pool.query('SELECT * FROM inventory_categories WHERE id = ?', [req.params.id]);
        if (old.length === 0)
            return error(res, 'Category not found', 404);
        await pool.query('UPDATE inventory_categories SET name=?, description=? WHERE id=?', [name, description, req.params.id]);
...
export const getInventoryItems = async (req, res) => {
    try {
        ...
        if (req.query.category_id) {
            filters += ' AND ii.category_id = ?';
            params.push(req.query.category_id);
        }
        if (req.query.low_stock === 'true') {
            filters += ' AND ii.quantity <= ii.min_stock_level';
        }
        if (req.query.location_id) {
            filters += ' AND ii.location_id = ?';
...
export const createInventoryItem = async (req, res) => {
    try {
        ...
        const [result] = await pool.query(`INSERT INTO inventory_items (category_id, name, code, barcode, unit, quantity, min_stock_level, max_stock_level, purchase_price, location_id, supplier_id) VALUES (?,?,?,?,?,?,?,?,?,?,?)`, [category_id, name, code, barcode, unit, quantity, min_stock_level, max_stock_level, purchase_price, location_id, supplier_id]);
...
export const updateInventoryItem = async (req, res) => {
    try {
        const { category_id, name, code, barcode, unit, quantity, min_stock_level, max_stock_level, purchase_price, location_id, supplier_id } = req.body;
        const [old] = await pool.query('SELECT * FROM inventory_items WHERE id = ?', [req.params.id]);
        if (old.length === 0)
            return error(res, 'Inventory item not found', 404);
        await pool.query(`UPDATE inventory_items SET category_id=?, name=?, code=?, barcode=?, unit=?, quantity=?, min_stock_level=?, max_stock_level=?, purchase_price=?, location_id=?, supplier_id=? WHERE id=?`, [category_id, name, code, barcode, unit, quantity, min_stock_level, max_stock_level, purchase_price, location_id, supplier_id, req.params.id]);
...
export const getLowStockItems = async (req, res) => {
    try {
        const [rows] = await pool.query(`SELECT ii.*, ic.name as category_name
       FROM inventory_items ii
       LEFT JOIN inventory_categories ic ON ii.category_id = ic.id
...
export const getStockValue = async (req, res) => {
    try {
        const [rows] = await pool.query(`SELECT SUM(ii.quantity * ii.purchase_price) as total_value,
              COUNT(*) as total_items,
              SUM(ii.quantity) as total_quantity
       FROM inventory_items ii
       WHERE ii.deleted_at IS NULL`);
        const [byCategory] = await pool.query(`SELECT ic.name as category, SUM(ii.quantity * ii.purchase_price) as value, COUNT(*) as items
       FROM inventory_items ii
       LEFT JOIN inventory_categories ic ON ii.category_id = ic.id
       WHERE ii.deleted_at IS NULL
       GROUP BY ic.name`);
