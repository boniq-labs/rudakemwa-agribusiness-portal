...
export const getAnimalCategories = async (req, res) => {
    try {
...
export const createAnimalCategory = async (req, res) => {
    try {
        const { name, description } = req.body;
        const [existing] = await pool.query('SELECT id FROM animal_categories WHERE name = ?', [name]);
        if (existing.length > 0)
            return error(res, 'Category already exists', 400);
        const [result] = await pool.query('INSERT INTO animal_categories (name, description) VALUES (?,?)', [name, description]);
...
export const updateAnimalCategory = async (req, res) => {
    try {
        const { name, description } = req.body;
        const [old] = await pool.query('SELECT * FROM animal_categories WHERE id = ?', [req.params.id]);
        if (old.length === 0)
            return error(res, 'Category not found', 404);
        await pool.query('UPDATE animal_categories SET name=?, description=? WHERE id=?', [name, description, req.params.id]);
...
export const createBreed = async (req, res) => {
    try {
        ...
        const [result] = await pool.query('INSERT INTO breeds (category_id, name, description) VALUES (?,?,?)', [category_id, name, description]);
...
export const updateBreed = async (req, res) => {
    try {
        const { category_id, name, description } = req.body;
        const [old] = await pool.query('SELECT * FROM breeds WHERE id = ?', [req.params.id]);
        if (old.length === 0)
            return error(res, 'Breed not found', 404);
        await pool.query('UPDATE breeds SET category_id=?, name=?, description=? WHERE id=?', [category_id, name, description, req.params.id]);
...
export const getAnimals = async (req, res) => {
    try {
        ...
        let filters = '';
        if (req.query.category_id) {
            filters += ' AND a.animal_category_id = ?';
            params.push(req.query.category_id);
        }
        if (req.query.breed_id) {
            filters += ' AND a.breed_id = ?';
            params.push(req.query.breed_id);
        }
        if (req.query.status) {
        ...
        if (req.query.gender) {
            filters += ' AND a.gender = ?';
            params.push(req.query.gender);
        }
        if (req.query.is_dairy) {
            filters += ' AND a.is_dairy = ?';
            params.push(req.query.is_dairy);
        }
...
export const getAnimalProfile = async (req, res) => {
    try {
        const [rows] = await pool.query(`SELECT a.*, ac.name as category_name, b.name as breed_name, l.name as location_name,
              (SELECT weight FROM weight_records WHERE animal_id = a.id ORDER BY date DESC LIMIT 1) as latest_weight
       FROM animals a
       JOIN animal_categories ac ON a.animal_category_id = ac.id
       LEFT JOIN breeds b ON a.breed_id = b.id
       LEFT JOIN animal_locations l ON a.location_id = l.id
       WHERE a.id = ? AND a.deleted_at IS NULL`, [req.params.id]);
        if (rows.length === 0)
            return error(res, 'Animal not found', 404);
...
export const createAnimal = async (req, res) => {
    try {
        ...
        const [existing] = await pool.query('SELECT id FROM animals WHERE tag_number = ?', [tag_number]);
        if (existing.length > 0)
            return error(res, 'Tag number already exists', 400);
        const [result] = await pool.query(`INSERT INTO animals (tag_number, name, animal_category_id, breed_id, gender, color, date_of_birth, weight, height, source, purchase_price, is_dairy, location_id, group_id, photo, feed_type, animal_status, status)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, [tag_number, name, animal_category_id, breed_id || null, gender, color, date_of_birth, weight, height, source, purchase_price, is_dairy ?? false, location_id || null, group_id || null, photo || null, feed_type || null, animal_status || null, 'active']);
...
export const updateAnimal = async (req, res) => {
    try {
        const [old] = await pool.query('SELECT * FROM animals WHERE id = ?', [req.params.id]);
        if (old.length === 0)
            return error(res, 'Animal not found', 404);
        const fields = [];
        const values = [];
...
export const getAnimalLocations = async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT id, name, type, capacity FROM animal_locations');
...
export const getAnimalGroups = async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT id, name, description FROM animal_groups WHERE deleted_at IS NULL');
        return success(res, rows);
    }
    catch (err) {
        return error(res, err.message);
