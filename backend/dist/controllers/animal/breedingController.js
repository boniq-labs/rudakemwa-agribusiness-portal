...
export const getBreedingRecords = async (req, res) => {
    try {
        ...
        if (req.query.animal_id) {
            filters += ' AND (mother_id = ? OR father_id = ?)';
            params.push(req.query.animal_id, req.query.animal_id);
        }
        if (req.query.status) {
            filters += ' AND br.status = ?';
...
export const createBreedingRecord = async (req, res) => {
    try {
        const { mother_id, father_id, breeding_date, method, notes } = req.body;
        const [result] = await pool.query(`INSERT INTO breeding_records (mother_id, father_id, breeding_date, method, notes) VALUES (?,?,?,?,?)`, [mother_id, father_id || null, breeding_date, method, notes]);
...
export const updateBreedingRecord = async (req, res) => {
    try {
        const [old] = await pool.query('SELECT * FROM breeding_records WHERE id = ?', [req.params.id]);
        if (old.length === 0)
            return error(res, 'Breeding record not found', 404);
        const fields = [];
        const values = [];
...
export const createPregnancy = async (req, res) => {
    try {
        ...
        const [result] = await pool.query(`INSERT INTO pregnancies (animal_id, breeding_record_id, pregnancy_date, expected_delivery_date, sire_name, status, notes) VALUES (?,?,?,?,?,?,?)`, [animal_id, breeding_record_id || null, pregnancy_date, expected_delivery_date, sire_name || null, status || 'confirmed', notes || null]);
...
export const updatePregnancy = async (req, res) => {
    try {
        const [old] = await pool.query('SELECT * FROM pregnancies WHERE id = ?', [req.params.id]);
        if (old.length === 0)
            return error(res, 'Pregnancy not found', 404);
        const fields = [];
        const values = [];
...
export const getBirthRecords = async (req, res) => {
    try {
        ...
        if (req.query.mother_id) {
            filters += ' AND br.mother_id = ?';
            params.push(req.query.mother_id);
        }
        if (req.query.start_date) {
            filters += ' AND br.birth_date >= ?';
        ...
        const dataQuery = `
      SELECT br.*, m.tag_number as mother_tag, m.name as mother_name, f.tag_number as father_tag, f.name as father_name,
             a.id as animal_id, a.tag_number as offspring_tag, a.name as offspring_name
...
export const createBirthRecord = async (req, res) => {
    ...
    try {
        ...
        const { mother_id, birth_date, weight, health_status, notes, photo, animal_name, tag_number, type } = req.body;
        const [mother] = await connection.query('SELECT tag_number, animal_category_id FROM animals WHERE id = ?', [mother_id]);
        const motherTag = mother[0]?.tag_number || 'UNKNOWN';
        const categoryId = mother[0]?.animal_category_id || null;
        const tagNum = tag_number || `${motherTag}-B${Date.now().toString().slice(-4)}`;
        const gender = type === 'Bull' ? 'male' : 'female';
        const [animalResult] = await connection.query(`INSERT INTO animals (tag_number, name, animal_category_id, gender, date_of_birth, weight, status, source, photo)
       VALUES (?,?,?,?,?,?,?,?,?)`, [tagNum, animal_name || `Offspring of ${motherTag}`, categoryId, gender, birth_date, weight, 'active', 'born', photo || null]);
        const animalId = animalResult.insertId;
        const [result] = await connection.query(`INSERT INTO birth_records (mother_id, animal_id, birth_date, gender, weight, health_status, notes, photo, tag_number, animal_name, type) VALUES (?,?,?,?,?,?,?,?,?,?,?)`, [mother_id, animalId, birth_date, gender, weight, health_status, notes || null, photo || null, tagNum, animal_name || null, type || null]);
        await connection.query('UPDATE pregnancies SET status = ? WHERE animal_id = ? AND status = ?', ['delivered', mother_id, 'confirmed']);
        ...
        try {
            await createNotification(req.user.id, 'success', 'Birth Recorded', `New birth: ${animal_name || tagNum} from ${motherTag}`);
        }
        catch { }
...
export const updateBirthRecord = async (req, res) => {
    try {
        const [old] = await pool.query('SELECT * FROM birth_records WHERE id = ?', [req.params.id]);
        if (old.length === 0)
            return error(res, 'Birth record not found', 404);
        const fields = [];
        const values = [];
        const allowed = ['mother_id', 'birth_date', 'gender', 'weight', 'health_status', 'notes'];
        for (const key of allowed)
            if (req.body[key] !== undefined) {
                fields.push(`${key}=?`);
                values.push(req.body[key]);
...
//# sourceMappingURL=breedingController.js.map