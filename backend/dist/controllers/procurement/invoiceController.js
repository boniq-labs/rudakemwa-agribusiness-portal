...
export const createProcurementInvoice = async (req, res) => {
    try {
        const { supplier_id, po_id, amount, due_date } = req.body;
        const invoice_number = req.body.invoice_number || `INV-${Date.now()}`;
        const [result] = await pool.query('INSERT INTO supplier_invoices (invoice_number, supplier_id, po_id, amount, total, due_date, status) VALUES (?,?,?,?,?,?,?)', [invoice_number, supplier_id, po_id, amount, amount, due_date || null, 'pending']);
...
export const payProcurementInvoice = async (req, res) => {
    try {
        const [old] = await pool.query('SELECT * FROM supplier_invoices WHERE id = ?', [req.params.id]);
        if (old.length === 0)
