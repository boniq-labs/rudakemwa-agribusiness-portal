...
export const createSupplierInvoice = async (req, res) => {
    try {
        ...
        const [result] = await pool.query(`INSERT INTO supplier_invoices (invoice_number, supplier_id, po_id, amount, total, due_date, status)
       VALUES (?,?,?,?,?,?,'pending')`, [invNo, supplier_id, po_id, amount, amount, due_date || null]);
...
export const recordSupplierPayment = async (req, res) => {
    try {
        ...
        const [inv] = await pool.query('SELECT * FROM supplier_invoices WHERE id = ?', [invoice_id]);
        if (inv.length === 0)
            return error(res, 'Invoice not found', 404);
        const [result] = await pool.query(`INSERT INTO supplier_payments (invoice_id, amount, payment_date, payment_method, reference_number)
       VALUES (?,?,?,?,?)`, [invoice_id, amount, payment_date, payment_method, reference_number]);
        const [paid] = await pool.query('SELECT COALESCE(SUM(amount),0) as total_paid FROM supplier_payments WHERE invoice_id = ?', [invoice_id]);
...
export const getSupplierContracts = async (req, res) => {
    try {
        const [rows] = await pool.query(`SELECT sc.*, s.supplier_name as supplier FROM supplier_contracts sc
...
export const createSupplierContract = async (req, res) => {
    try {
        ...
        const [result] = await pool.query(`INSERT INTO supplier_contracts (supplier_id, contract_number, title, description, start_date, end_date, value, terms, status)
       VALUES (?,?,?,?,?,?,?,?,'active')`, [supplier_id, contract_number, title, description || null, start_date, end_date, value, terms || null]);
...
export const updateSupplierContract = async (req, res) => {
    try {
        const { supplier_id, contract_number, title, description, start_date, end_date, value, terms, status } = req.body;
        const [old] = await pool.query('SELECT * FROM supplier_contracts WHERE id = ? AND deleted_at IS NULL', [req.params.id]);
        if (old.length === 0)
            return error(res, 'Contract not found', 404);
        await pool.query(`UPDATE supplier_contracts SET supplier_id=?, contract_number=?, title=?, description=?, start_date=?, end_date=?, value=?, terms=?, status=? WHERE id=?`, [supplier_id, contract_number, title, description, start_date, end_date, value, terms, status, req.params.id]);
...
//# sourceMappingURL=supplierInvoiceController.js.map