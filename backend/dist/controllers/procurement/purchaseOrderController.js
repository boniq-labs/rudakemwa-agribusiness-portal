...
export const createPurchaseOrder = async (req, res) => {
    try {
        ...
        const [result] = await pool.query('INSERT INTO purchase_orders (po_number, supplier_id, request_id, order_date, expected_delivery, notes, status, created_by) VALUES (?,?,?,?,?,?,?,?)', [po_number, supplier_id, request_id || null, order_date, expected_delivery || null, notes || null, 'pending', req.user?.id]);
        const orderId = result.insertId;
        if (items && items.length > 0) {
            for (const item of items) {
                const total_price = (item.quantity || 0) * (item.unit_price || 0);
                await pool.query('INSERT INTO purchase_order_items (po_id, item_name, description, quantity, unit, unit_price, total_price) VALUES (?,?,?,?,?,?,?)', [orderId, item.item_name, item.description || null, item.quantity, item.unit || null, item.unit_price || 0, total_price]);
            }
        }
...
export const updatePurchaseOrder = async (req, res) => {
    try {
        ...
        const [old] = await pool.query('SELECT * FROM purchase_orders WHERE id = ? AND deleted_at IS NULL', [req.params.id]);
        if (old.length === 0)
            return error(res, 'Purchase order not found', 404);
        await pool.query('UPDATE purchase_orders SET supplier_id=?, request_id=?, order_date=?, expected_delivery=?, status=?, notes=? WHERE id=?', [supplier_id, request_id || null, order_date, expected_delivery || null, status || 'draft', notes || null, req.params.id]);
...
export const receivePurchaseOrder = async (req, res) => {
    try {
        ...
        for (const item of items[0]) {
            if (item.item_id) {
                await pool.query('UPDATE inventory_items SET quantity = quantity + ? WHERE id = ?', [item.quantity, item.item_id]);
            }
        }
