...
const generateTokens = (user) => {
    const tokenOpts = { expiresIn: (process.env.JWT_EXPIRES_IN || '1h') };
    const refreshOpts = { expiresIn: (process.env.JWT_REFRESH_EXPIRES_IN || '7d') };
    const token = jwt.sign({ id: user.id, username: user.username, role: user.role }, process.env.JWT_SECRET || 'secret', tokenOpts);
    const refreshToken = jwt.sign({ id: user.id }, process.env.JWT_REFRESH_SECRET || 'refresh_secret', refreshOpts);
...
export const login = async (req, res) => {
    try {
        ...
        const [users] = await pool.query(`SELECT u.*, r.slug as role, r.name as role_name, d.name as department_name
       FROM users u JOIN roles r ON u.role_id = r.id
       LEFT JOIN departments d ON u.department_id = d.id
       WHERE (u.username = ? OR u.email = ?) AND u.deleted_at IS NULL`, [username, username]);
        if (users.length === 0)
        ...
        const [permissions] = await pool.query(`SELECT p.slug FROM permissions p JOIN role_permissions rp ON p.id = rp.permission_id WHERE rp.role_id = ?`, [user.role_id]);
        return success(res, {
            token, refreshToken,
            user: {
...
export const refreshTokenHandler = async (req, res) => {
    try {
        ...
        const [users] = await pool.query('SELECT * FROM users WHERE id = ? AND refresh_token = ?', [payload.id, refreshToken]);
        if (users.length === 0)
...
export const getProfile = async (req, res) => {
    try {
        const [users] = await pool.query(`SELECT u.id, u.username, u.email, u.first_name, u.last_name, u.photo, u.phone,
...
export const updateProfile = async (req, res) => {
    try {
       ...
       FROM users u JOIN roles r ON u.role_id = r.id
       LEFT JOIN departments d ON u.department_id = d.id WHERE u.id = ?`, [req.user.id]);
        return success(res, {
