...
export const updateProductCategory = async (req, res) => {
    try {
        const [old] = await pool.query('SELECT * FROM product_categories WHERE id = ?', [req.params.id]);
        if (old.length === 0)
...
export const getProducts = async (req, res) => {
    try {
        ...
        const [rows] = await pool.query(`SELECT p.*, pc.name as category_name
...
export const createProduct = async (req, res) => {
    try {
        ...
        const [result] = await pool.query(`INSERT INTO products (category_id, name, code, unit, price, cost_price, quantity_available, description)
       VALUES (?,?,?,?,?,?,?,?)`, [category_id || null, name, code || null, unit || null, price, cost_price || null, quantity_available, description || null]);
...
export const updateProduct = async (req, res) => {
    try {
        ...
        await pool.query(`UPDATE products SET category_id=?, name=?, code=?, unit=?, price=?, cost_price=?, quantity_available=?, description=?, status=? WHERE id=?`, [category_id, name, code || null, unit || null, price, cost_price || null, quantity_available, description || null, status || old[0].status, req.params.id]);
