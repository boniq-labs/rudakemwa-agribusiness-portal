...
export const getSalesOrders = async (req, res) => {
    try {
        ...
        const [rows] = await pool.query(`SELECT so.*, CONCAT(c.first_name, ' ', c.last_name) as customer_name, c.company_name
...
export const createSalesOrder = async (req, res) => {
    try {
        ...
        const total_amount = b.total_amount || items.reduce((sum, item) => sum + (item.quantity * item.unit_price), 0);
        const order_date = b.order_date || new Date().toISOString().split('T')[0];
        const [result] = await pool.query(`INSERT INTO sales_orders (order_number, customer_id, total_amount, order_date, notes) VALUES (?,?,?,?,?)`, [order_number, b.customer_id, total_amount, order_date, b.notes || null]);
        for (const item of items) {
            await pool.query(`INSERT INTO sales_order_items (order_id, product_id, quantity, unit_price, total_price) VALUES (?,?,?,?,?)`, [result.insertId, item.product_id, item.quantity, item.unit_price, item.quantity * item.unit_price]);
        }
...
export const updateSalesOrderStatus = async (req, res) => {
    try {
        ...
        if (status === 'completed') {
            const [items] = await pool.query('SELECT * FROM sales_order_items WHERE order_id = ?', [req.params.id]);
            for (const item of items) {
...
export const getQuotations = async (req, res) => {
    try {
        ...
        const [rows] = await pool.query(`SELECT q.*, CONCAT(c.first_name, ' ', c.last_name) as customer_name, c.company_name
...
export const createQuotation = async (req, res) => {
    try {
        ...
        const [result] = await pool.query(`INSERT INTO sales_quotations (quotation_number, customer_id, total_amount, notes, valid_until) VALUES (?,?,?,?,?)`, [qNumber, customer_id, total_amount, notes || null, valid_until || null]);
        for (const item of items || []) {
            await pool.query(`INSERT INTO sales_quotation_items (quotation_id, product_id, description, quantity, unit_price, total_price) VALUES (?,?,?,?,?,?)`, [result.insertId, item.product_id, item.description || null, item.quantity, item.unit_price, item.quantity * item.unit_price]);
        }
...
export const convertQuotationToOrder = async (req, res) => {
    try {
        const [quotation] = await pool.query('SELECT * FROM sales_quotations WHERE id = ?', [req.params.id]);
        if (quotation.length === 0)
            return error(res, 'Quotation not found', 404);
        const [qItems] = await pool.query('SELECT * FROM sales_quotation_items WHERE quotation_id = ?', [req.params.id]);
        const orderNumber = `ORD-${Date.now()}`;
        const [orderResult] = await pool.query(`INSERT INTO sales_orders (order_number, customer_id, total_amount, notes) VALUES (?,?,?,?)`, [orderNumber, quotation[0].customer_id, quotation[0].total_amount, quotation[0].notes]);
        for (const item of qItems) {
            await pool.query(`INSERT INTO sales_order_items (order_id, product_id, quantity, unit_price, total_price) VALUES (?,?,?,?,?)`, [orderResult.insertId, item.product_id, item.quantity, item.unit_price, item.total_price]);
        }
...
export const getSalesReturns = async (req, res) => {
    try {
        ...
        const params = [];
        if (order_id) {
            where += ' AND sr.order_id = ?';
            params.push(order_id);
        }
        const [rows] = await pool.query(`SELECT sr.*, so.order_number
...
export const createSalesReturn = async (req, res) => {
    try {
        ...
        const [order] = await pool.query('SELECT * FROM sales_orders WHERE id = ?', [order_id]);
        if (order.length === 0)
            return error(res, 'Order not found', 404);
        const [result] = await pool.query(`INSERT INTO sales_returns (return_number, order_id, reason, return_date) VALUES (?,?,?,CURDATE())`, [`RET-${Date.now()}`, order_id, reason || null]);
        for (const item of items || []) {
            await pool.query(`INSERT INTO sales_return_items (return_id, product_id, quantity, reason) VALUES (?,?,?,?)`, [result.insertId, item.product_id, item.quantity, item.reason || reason]);
            await pool.query('UPDATE products SET quantity_available = quantity_available + ? WHERE id = ?', [item.quantity, item.product_id]);
        }
