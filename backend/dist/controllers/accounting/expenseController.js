...
export const createExpenseCategory = async (req, res) => {
    try {
        ...
        const [result] = await pool.query('INSERT INTO expense_categories (name, description) VALUES (?,?)', [name, description]);
...
export const updateExpenseCategory = async (req, res) => {
    try {
        const [old] = await pool.query('SELECT * FROM expense_categories WHERE id = ?', [req.params.id]);
        if (old.length === 0)
...
export const getExpenseRecords = async (req, res) => {
    try {
        ...
        const [rows] = await pool.query(`SELECT e.*, ec.name as category_name, d.name as department_name
...
async function getExpenseCategoryId(b) {
    if (b.category_id)
        return Number(b.category_id);
    if (b.category) {
        if (!isNaN(Number(b.category)))
            return Number(b.category);
        const [rows] = await (await import('../../config/database')).default.query('SELECT id FROM expense_categories WHERE name = ?', [b.category]);
        if (rows.length > 0)
            return rows[0].id;
    }
    return null;
...
export const createExpenseRecord = async (req, res) => {
    try {
        ...
        const [result] = await pool.query(`INSERT INTO expense_records (expense_number, category_id, description, amount, payment_method, vendor, notes, date, department_id) VALUES (?,?,?,?,?,?,?,?,?)`, [genExpenseNumber, category_id, b.description || null, b.amount, payment_method || null, b.vendor || null, b.notes || null, b.date, b.department_id || null]);
...
export const updateExpenseRecord = async (req, res) => {
    try {
        const [old] = await pool.query('SELECT * FROM expense_records WHERE id = ?', [req.params.id]);
        if (old.length === 0)
            return error(res, 'Expense record not found', 404);
        const b = req.body;
        const category_id = await getExpenseCategoryId(b);
        const payment_method = (b.payment_method || '').toLowerCase().replace(/\s+/g, '_');
        await pool.query(`UPDATE expense_records SET category_id=?, description=?, amount=?, payment_method=?, vendor=?, notes=?, date=?, department_id=? WHERE id=?`, [category_id, b.description || null, b.amount, payment_method || null, b.vendor || null, b.notes || null, b.date, b.department_id || null, req.params.id]);
...
export const getExpenseSummary = async (req, res) => {
    try {
        ...
        const [rows] = await pool.query(`SELECT DATE_FORMAT(e.date, '${format}') as period${selectGroup}, SUM(e.amount) as total, COUNT(*) as count
