...
export const getBudgets = async (req, res) => {
    try {
        ...
        if (fiscal_year) {
            where += ' AND b.fiscal_year = ?';
            params.push(fiscal_year);
        }
        const [rows] = await pool.query(`SELECT b.*, d.name as department_name
...
export const createBudget = async (req, res) => {
    try {
        ...
        const [result] = await pool.query(`INSERT INTO budgets (name, department_id, fiscal_year, total_amount) VALUES (?,?,?,?)`, [name, department_id, fiscal_year, total_amount]);
        for (const item of items || []) {
            const catId = item.category_id || item.expense_category_id;
            await pool.query(`INSERT INTO budget_items (budget_id, category_id, planned_amount) VALUES (?,?,?)`, [result.insertId, catId, item.planned_amount]);
        }
...
export const getBudgetVsActual = async (req, res) => {
    try {
        const [budget] = await pool.query(`SELECT b.*, d.name as department_name
       FROM budgets b LEFT JOIN departments d ON b.department_id = d.id WHERE b.id = ?`, [req.params.id]);
        if (budget.length === 0)
            return error(res, 'Budget not found', 404);
        const [items] = await pool.query(`SELECT bi.*, ec.name as category_name,
