...
export const createProcessingRecord = async (req, res) => {
    try {
        const { collection_id, product_id, input_quantity, output_quantity, processing_date } = req.body;
        const [result] = await pool.query(`INSERT INTO milk_processing_records (collection_id, product_id, input_quantity, output_quantity, processing_date) VALUES (?,?,?,?,?)`, [collection_id, product_id, input_quantity, output_quantity, processing_date]);
...
export const createMilkProduct = async (req, res) => {
    try {
        const { name, code, description, unit, price, category } = req.body;
        const [result] = await pool.query(`INSERT INTO milk_products (name, code, description, unit, price, category) VALUES (?,?,?,?,?,?)`, [name, code, description, unit, price, category]);
...
export const updateMilkProduct = async (req, res) => {
    try {
        const { name, code, description, unit, price, category } = req.body;
        const [old] = await pool.query('SELECT * FROM milk_products WHERE id = ?', [req.params.id]);
        if (old.length === 0)
            return error(res, 'Milk product not found', 404);
        await pool.query(`UPDATE milk_products SET name=?, code=?, description=?, unit=?, price=?, category=? WHERE id=?`, [name, code, description, unit, price, category, req.params.id]);
...
//# sourceMappingURL=milkProcessingController.js.map