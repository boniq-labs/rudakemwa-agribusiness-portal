...
export const getMilkCollections = async (req, res) => {
    try {
        ...
        if (req.query.start_date) {
            filters += ' AND mc.collection_date >= ?';
...
export const createMilkCollection = async (req, res) => {
    try {
        ...
        const [result] = await pool.query(`INSERT INTO milk_collections (collection_date, time, collector_id, branch_id, quantity_liters, number_of_animals, notes) VALUES (?,?,?,?,?,?,?)`, [collection_date, time, collector_id, branch_id, quantity_liters, number_of_animals, notes]);
        ...
        try {
            await createNotification(req.user.id, 'info', 'Milk Production', `${quantity_liters}L of milk collected (${time})`);
        }
        catch { }
...
export const updateMilkCollection = async (req, res) => {
    try {
        const { collection_date, time, collector_id, branch_id, quantity_liters, number_of_animals, notes } = req.body;
        const [old] = await pool.query('SELECT * FROM milk_collections WHERE id = ?', [req.params.id]);
        if (old.length === 0)
            return error(res, 'Milk collection not found', 404);
        await pool.query(`UPDATE milk_collections SET collection_date=?, time=?, collector_id=?, branch_id=?, quantity_liters=?, number_of_animals=?, notes=? WHERE id=?`, [collection_date, time, collector_id, branch_id, quantity_liters, number_of_animals, notes, req.params.id]);
...
export const getDailyProduction = async (req, res) => {
    try {
        ...
        const [rows] = await pool.query(`SELECT time, SUM(quantity_liters) as total_liters, COUNT(*) as collection_count,
              SUM(number_of_animals) as total_animals
       FROM milk_collections
       WHERE collection_date = ? AND deleted_at IS NULL
       GROUP BY time`, [date]);
        const morning = rows.find((r) => r.time === 'morning');
        const evening = rows.find((r) => r.time === 'evening');
        return success(res, {
            date,
...
export const getMonthlyProduction = async (req, res) => {
    try {
        const { month } = req.query;
        const [rows] = await pool.query(`SELECT collection_date, time, SUM(quantity_liters) as total_liters
       FROM milk_collections
       WHERE DATE_FORMAT(collection_date, '%Y-%m') = ? AND deleted_at IS NULL
       GROUP BY collection_date, time
       ORDER BY collection_date ASC`, [month]);
        const daily = {};
        for (const row of rows) {
            const d = row.collection_date;
            if (!daily[d])
                daily[d] = { date: d, morning: 0, evening: 0, total: 0 };
...
//# sourceMappingURL=milkCollectionController.js.map