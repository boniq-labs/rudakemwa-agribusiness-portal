...
export const createPosition = async (req, res) => {
    try {
        ...
        if (!name)
            return error(res, 'Position name is required', 400);
        const [result] = await pool.query(`INSERT INTO positions (name, department_id, description, requirements) VALUES (?,?,?,?)`, [name, department_id || null, description || null, requirements || null]);
...
export const updatePosition = async (req, res) => {
    try {
        ...
        const [old] = await pool.query('SELECT * FROM positions WHERE id = ? AND deleted_at IS NULL', [req.params.id]);
        if (old.length === 0)
            return error(res, 'Position not found', 404);
        await pool.query(`UPDATE positions SET name=?, department_id=?, description=?, requirements=? WHERE id=?`, [name, department_id || null, description || null, requirements || null, req.params.id]);
