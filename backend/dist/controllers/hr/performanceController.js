...
export const createPerformanceReview = async (req, res) => {
    try {
        ...
        if (b.user_id && !b.employee_id) {
            const [emp] = await pool.query('SELECT id FROM employees WHERE user_id = ?', [b.user_id]);
            if (emp.length > 0)
                employeeId = emp[0].id;
        }
        ...
        if (!reviewerId) {
            ...
            if (b.reviewer_name && !reviewerId) {
                const [userRow] = await pool.query('SELECT id FROM users WHERE CONCAT(first_name, " ", last_name) = ? OR first_name = ? LIMIT 1', [b.reviewer_name, b.reviewer_name]);
                if (userRow.length > 0)
                    reviewerId = userRow[0].id;
            }
        }
        if (!reviewerId)
            return error(res, 'Reviewer information is required', 400);
        ...
        if (!reviewDate)
            return error(res, 'Review date is required', 400);
        const [result] = await pool.query(`INSERT INTO performance_reviews (employee_id, reviewer_id, score, rating, comments, goals)
       VALUES (?,?,?,?,?,?)`, [employeeId, reviewerId, b.score ? Number(b.score) : null, rating, notes || null, goals ? JSON.stringify(goals) : null]);
...
export const updatePerformanceReview = async (req, res) => {
    try {
        ...
        const [old] = await pool.query('SELECT * FROM performance_reviews WHERE id = ?', [req.params.id]);
        if (old.length === 0)
            return error(res, 'Performance review not found', 404);
        await pool.query(`UPDATE performance_reviews SET score=?, rating=?, comments=?, goals=? WHERE id=?`, [score, rating, comments || null, goals ? JSON.stringify(goals) : null, req.params.id]);
