...
export const createContract = async (req, res) => {
    try {
        ...
        if (!contract_type)
            return error(res, 'Contract type is required', 400);
        if (!start_date)
            return error(res, 'Start date is required', 400);
        const [result] = await pool.query(`INSERT INTO contracts (employee_id, contract_type, start_date, end_date, salary, document_url, status)
       VALUES (?,?,?,?,?,?,'active')`, [employeeId, contract_type, start_date, end_date, salary, document_url]);
...
export const updateContract = async (req, res) => {
    try {
        ...
        const [old] = await pool.query('SELECT * FROM contracts WHERE id = ?', [req.params.id]);
        if (old.length === 0)
            return error(res, 'Contract not found', 404);
        await pool.query(`UPDATE contracts SET type=?, start_date=?, end_date=?, salary=?, terms=?, status=? WHERE id=?`, [type, start_date, end_date || null, salary, terms || null, status, req.params.id]);
...
export const terminateContract = async (req, res) => {
    try {
        ...
        if (old.length === 0)
            return error(res, 'Contract not found', 404);
        if (old[0].status === 'terminated')
            return error(res, 'Contract already terminated', 400);
...
export const getExpiringContracts = async (req, res) => {
    try {
        const [rows] = await pool.query(`SELECT c.*, e.employee_code, CONCAT(u.first_name, ' ', u.last_name) as employee_name
       FROM contracts c JOIN employees e ON c.employee_id = e.id
