...
export const createTraining = async (req, res) => {
    try {
        ...
        const [result] = await pool.query(`INSERT INTO trainings (title, description, trainer, start_date, end_date, status)
       VALUES (?,?,?,?,?,?)`, [title, description || null, trainer || null, start, end_date || null, status || 'planned']);
...
export const updateTraining = async (req, res) => {
    try {
        const { title, description, trainer, start_date, end_date, status } = req.body;
        const [old] = await pool.query('SELECT * FROM trainings WHERE id = ?', [req.params.id]);
        if (old.length === 0)
            return error(res, 'Training not found', 404);
        await pool.query(`UPDATE trainings SET title=?, description=?, trainer=?, start_date=?, end_date=?, status=? WHERE id=?`, [title, description || null, trainer || null, start_date, end_date || null, status, req.params.id]);
...
export const getTrainingParticipants = async (req, res) => {
    try {
        const [rows] = await pool.query(`SELECT tp.*, e.employee_code, CONCAT(u.first_name, ' ', u.last_name) as employee_name
       FROM training_participants tp
       JOIN employees e ON tp.employee_id = e.id
       JOIN users u ON e.user_id = u.id
       WHERE tp.training_id = ?
       ORDER BY tp.enrolled_at`, [req.params.training_id]);
        return success(res, rows);
    }
    catch (err) {
        return error(res, err.message);
...
export const enrollParticipant = async (req, res) => {
    try {
        ...
        if (!employeeId && req.body.userId) {
            const [emp] = await pool.query('SELECT id FROM employees WHERE user_id = ?', [req.body.userId]);
            if (emp.length > 0)
                employeeId = emp[0].id;
        }
        if (!trainingId)
            return error(res, 'Training ID is required', 400);
        if (!employeeId)
            return error(res, 'Employee ID is required', 400);
        const [existing] = await pool.query('SELECT id FROM training_participants WHERE training_id = ? AND employee_id = ?', [trainingId, employeeId]);
        if (existing.length > 0)
            return error(res, 'Employee already enrolled', 400);
        const [result] = await pool.query(`INSERT INTO training_participants (training_id, employee_id, status) VALUES (?,?,'enrolled')`, [trainingId, employeeId]);
...
export const updateParticipantStatus = async (req, res) => {
    try {
        ...
        const [old] = await pool.query('SELECT * FROM training_participants WHERE id = ?', [req.params.id]);
        if (old.length === 0)
            return error(res, 'Participant not found', 404);
        await pool.query(`UPDATE training_participants SET status=?, score=?, certificate_url=?, completed_at=? WHERE id=?`, [status, score || null, certificate_url || null, status === 'completed' ? new Date() : null, req.params.id]);
