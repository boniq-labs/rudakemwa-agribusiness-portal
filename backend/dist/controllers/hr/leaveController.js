...
export const getLeaveTypes = async (req, res) => {
    try {
        const [rows] = await pool.query('SELECT * FROM leave_types WHERE deleted_at IS NULL ORDER BY name');
...
export const createLeaveType = async (req, res) => {
    try {
        ...
        const [result] = await pool.query(`INSERT INTO leave_types (name, days_allowed, requires_approval) VALUES (?,?,?)`, [name, days_allowed, requires_approval ?? true]);
...
export const updateLeaveType = async (req, res) => {
    try {
        const { name, days_allowed, requires_approval } = req.body;
        const [old] = await pool.query('SELECT * FROM leave_types WHERE id = ?', [req.params.id]);
        if (old.length === 0)
            return error(res, 'Leave type not found', 404);
        await pool.query(`UPDATE leave_types SET name=?, days_allowed=?, requires_approval=? WHERE id=?`, [name, days_allowed, requires_approval, req.params.id]);
...
export const createLeaveRequest = async (req, res) => {
    try {
        ...
        const [result] = await pool.query(`INSERT INTO leave_requests (employee_id, leave_type_id, start_date, end_date, reason, status)
       VALUES (?,?,?,?,?,'pending')`, [employee_id, leave_type_id, start_date, end_date, reason || null]);
...
export const approveLeave = async (req, res) => {
    try {
        const [old] = await pool.query('SELECT * FROM leave_requests WHERE id = ?', [req.params.id]);
        if (old.length === 0)
            return error(res, 'Leave request not found', 404);
        if (old[0].status !== 'pending')
            return error(res, 'Leave request is not pending', 400);
        await pool.query('UPDATE leave_requests SET status=?, approved_by=?, approved_at=NOW() WHERE id=?', ['approved', req.user?.id, req.params.id]);
...
export const cancelLeave = async (req, res) => {
    try {
        ...
        if (old[0].status === 'cancelled')
            return error(res, 'Leave request already cancelled', 400);
