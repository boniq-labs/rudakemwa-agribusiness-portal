...
async function seed() {
    ...
    const [ownerRole] = await conn.query('SELECT id FROM roles WHERE slug = "farm_owner"');
    ...
    const [deptMgmt] = await conn.query('SELECT id FROM departments WHERE name = "Management"');
    await conn.query('DELETE FROM users WHERE username IN ("owner","admin","ruda")');
    await conn.query('INSERT INTO users (username, password, email, first_name, last_name, role_id, department_id) VALUES (?,?,?,?,?,?,?)', ['ruda', hashed, 'ruda@farm.com', 'Ruda', '', ownerRole[0].id, deptMgmt[0].id]);
    const [ownerUser] = await conn.query('SELECT id FROM users WHERE username = "ruda"');
    await conn.query('INSERT IGNORE INTO employees (user_id, employee_code, position, date_hired) VALUES (?,?,?,CURDATE())', [ownerUser[0].id, 'RUDA001', 'Farm Owner']);
    ...
    for (const [slug] of Object.entries(rolePerms)) {
        const [role] = await conn.query('SELECT id FROM roles WHERE slug = ?', [slug]);
        if (role.length > 0) {
    ...
    for (const [slug, mods] of Object.entries(rolePerms)) {
        ...
        for (const mod of mods) {
            ...
            for (const action of actionList) {
                const [perm] = await conn.query('SELECT id FROM permissions WHERE slug = ?', [`${mod}.${action}`]);
                if (perm.length > 0) {
    ...
    const [hrRole] = await conn.query('SELECT id FROM roles WHERE slug = "hr"');
    if (hrRole.length > 0) {
        const [userViewPerm] = await conn.query('SELECT id FROM permissions WHERE slug = "users.view"');
        ...
        const [userExportPerm] = await conn.query('SELECT id FROM permissions WHERE slug = "users.export"');
