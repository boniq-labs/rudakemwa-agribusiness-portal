...
import { createPositionSchema, createAttendanceSchema, createLeaveTypeSchema, createLeaveRequestSchema, createContractSchema, updateContractSchema, createPerformanceReviewSchema, createTrainingSchema, createJobSchema, createApplicantSchema, createAnimalCategorySchema, createAnimalSchema, updateAnimalSchema, createBreedingRecordSchema, createPregnancySchema, createBirthRecordSchema, createVaccinationSchema, createDiseaseSchema, createTreatmentSchema, createFeedingRecordSchema, createWeightRecordSchema, createMilkCollectionSchema, createQualityTestSchema, createStorageTankSchema, createInventoryCategorySchema, createInventoryItemSchema, receiveStockSchema, issueStockSchema, createSupplierSchema, createPurchaseRequestSchema, createPurchaseOrderSchema, createVehicleSchema, createDriverSchema, createTransportRequestSchema, createTripSchema, createFuelRecordSchema, createMaintenanceSchema, createIncomeSchema, createExpenseSchema, createInvoiceSchema, createCustomerSchema, createProductSchema, createSalesOrderSchema, createHealthRecordSchema, createVaccinationRecordSchema, createRoleSchema, } from '../validators/modules';
import { login, refreshTokenHandler, getProfile, changePassword, logout, updateProfile } from '../controllers/authController';
import { getDashboard, getDepartmentOverview } from '../controllers/dashboardController';
...
import { getAttendance, checkIn, checkOut, getAttendanceReport } from '../controllers/hr/attendanceController';
...
import { getBreedingRecords, createBreedingRecord, updateBreedingRecord, getPregnancies, createPregnancy, updatePregnancy, deletePregnancy, getBirthRecords, createBirthRecord, updateBirthRecord, deleteBirthRecord } from '../controllers/animal/breedingController';
...
import { getQualityTests, createQualityTest, deleteQualityTest, getQualityAlerts } from '../controllers/milk/milkQualityController';
...
import { getProcessingRecords, createProcessingRecord, getMilkProducts, createMilkProduct, updateMilkProduct } from '../controllers/milk/milkProcessingController';
import { getWasteRecords, createWasteRecord } from '../controllers/milk/milkWasteController';
...
import { receiveStock, issueStock, transferStock, adjustStock, getStockMovements } from '../controllers/stock/stockTransactionController';
...
import { getProductCategories, createProductCategory, getProducts, createProduct, updateProduct, deleteProduct, updateProductStock } from '../controllers/sales/productController';
...
import { getInvoices, createInvoice, updateInvoiceStatus, recordPayment } from '../controllers/accounting/invoiceController';
import { getPayrollRecords, createPayroll, processPayrollPayment, getSalaryRecords, createSalaryRecord } from '../controllers/accounting/payrollController';
...
router.get('/dashboard/hr', authenticate, hasRole('owner', 'admin', 'hr'), getDashboard);
router.get('/dashboard/animals', authenticate, hasRole('owner', 'admin', 'animal', 'veterinarian'), getDashboard);
router.get('/dashboard/animal', authenticate, hasRole('owner', 'admin', 'animal', 'veterinarian'), getDashboard);
router.get('/dashboard/milk', authenticate, hasRole('owner', 'admin', 'milk'), getDashboard);
router.get('/dashboard/stock', authenticate, hasRole('owner', 'admin', 'stock'), getDashboard);
router.get('/dashboard/procurement', authenticate, hasRole('owner', 'admin', 'procurement'), getDashboard);
router.get('/dashboard/logistics', authenticate, hasRole('owner', 'admin', 'logistics'), getDashboard);
router.get('/dashboard/accounting', authenticate, hasRole('owner', 'admin', 'accountant'), getDashboard);
router.get('/dashboard/accountant', authenticate, hasRole('owner', 'admin', 'accountant'), getDashboard);
router.get('/dashboard/sales', authenticate, hasRole('owner', 'admin', 'sales'), getDashboard);
router.get('/dashboard/veterinary', authenticate, hasRole('owner', 'admin', 'veterinarian'), getDashboard);
router.get('/dashboard/veterinarian', authenticate, hasRole('owner', 'admin', 'veterinarian'), getDashboard);
...
router.post('/attendance/check-out', authenticate, authorize(['attendance.update']), async (req, res, next) => {
    ...
    if (req.body.user_id) {
        const [rows] = await pool.query('SELECT id FROM attendance WHERE user_id = ? AND date = CURDATE() AND check_out IS NULL LIMIT 1', [req.body.user_id]);
        if (rows.length > 0)
...
router.get('/milk/reports', authenticate, async (req, res) => {
    const pool = (await import('../config/database')).default;
    const start_date = req.query.start_date || new Date(Date.now() - 7 * 86400000).toISOString().split('T')[0];
    const end_date = req.query.end_date || new Date().toISOString().split('T')[0];
    try {
        const [daily] = await pool.query(`SELECT collection_date as date,
       ...
       GROUP BY collection_date ORDER BY collection_date`, [start_date, end_date]);
        const [quality] = await pool.query(`SELECT * FROM milk_quality_tests WHERE test_date BETWEEN ? AND ? AND deleted_at IS NULL ORDER BY test_date`, [start_date, end_date]);
        const [waste] = await pool.query(`SELECT * FROM milk_waste WHERE date BETWEEN ? AND ? AND deleted_at IS NULL ORDER BY date`, [start_date, end_date]);
        const { success } = await import('../utils/response');
...
router.post('/accounting/payroll/:id/process', authenticate, authorize(['payroll.update']), processPayrollPayment);
...
router.get('/accounting/reports', authenticate, authorize(['reports.view']), async (req, res) => {
    ...
    try {
        const [incomeRows] = await pool.query(`SELECT source, SUM(amount) as total FROM income_records${dateWhere} GROUP BY source ORDER BY total DESC`, params);
        const incomeBySource = {};
        for (const r of incomeRows)
            incomeBySource[r.source] = Number(r.total);
        const [expenseRows] = await pool.query(`SELECT ec.name as category, SUM(e.amount) as total FROM expense_records e LEFT JOIN expense_categories ec ON e.category_id = ec.id${dateWhere.replace('date', 'e.date')} GROUP BY ec.name ORDER BY total DESC`, params.length ? params : []);
        const expenseByCategory = {};
        for (const r of expenseRows)
            expenseByCategory[r.category || 'Uncategorized'] = Number(r.total);
        const { success } = await import('../utils/response');
...
router.delete('/sales/orders/:id', authenticate, authorize(['orders.delete']), async (req, res) => {
    ...
    try {
        ...
        if (old.length === 0)
            return (await import('../utils/response')).error(res, 'Order not found', 404);
        ...
        return (await import('../utils/response')).error(res, err.message);
    }
});
// Sales - Quotations
...
router.put('/sales/invoices/:id', authenticate, authorize(['sales_invoices.update']), async (req, res) => {
    const pool = (await import('../config/database')).default;
    try {
        const [old] = await pool.query('SELECT * FROM sales_invoices WHERE id = ?', [req.params.id]);
        if (old.length === 0)
            return (await import('../utils/response')).error(res, 'Invoice not found', 404);
        const { invoice_date, due_date, total_amount, status, notes } = req.body;
        await pool.query('UPDATE sales_invoices SET invoice_date=?, due_date=?, total_amount=?, status=?, notes=? WHERE id=?', [invoice_date || null, due_date || null, total_amount, status || old[0].status, notes || null, req.params.id]);
...
router.post('/stock/equipment/maintenance', authenticate, authorize(['equipment.update']), (req, res, next) => { const { default: pool } = require('../config/database'); (async () => { if (req.body.equipment_id)
    req.params.id = req.body.equipment_id; next(); })(); }, createEquipmentMaintenance);
...
router.get('/accounting/profit-loss', authenticate, authorize(['reports.view']), getProfitLoss);
