...
export const createJob = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!posId && title) {
      ...
      const [existing]: any = await pool.query('SELECT id FROM positions WHERE name = ?', [title]);
      if (existing.length > 0) {
        posId = existing[0].id;
      } else {
        // Create a default position (department_id=1 = Management as fallback)
        const [newPos]: any = await pool.query('INSERT INTO positions (name, department_id, description) VALUES (?,?,?)', [title, 1, description || 'Auto-created for recruitment']);
        posId = newPos.insertId;
      }
    }
    const [result]: any = await pool.query(
      `INSERT INTO recruitment_jobs (title, position_id, description, requirements, closing_date, status)
       VALUES (?,?,?,?,?,?)`,
      [title, posId, description || null, requirements || null, closing_date || null, status || 'open']
    );
...
export const updateJob = async (req: AuthRequest, res: Response) => {
  try {
    const { title, position_id, description, requirements, closing_date, status } = req.body;
    const [old]: any = await pool.query('SELECT * FROM recruitment_jobs WHERE id = ?', [req.params.id]);
    if (old.length === 0) return error(res, 'Job not found', 404);
    ...
    if (!posId && title && title !== old[0].title) {
      ...
      if (existing.length > 0) {
        ...
        const [newPos]: any = await pool.query('INSERT INTO positions (name, department_id) VALUES (?,?)', [title, 1]);
        posId = newPos.insertId;
      }
    }

    await pool.query(
      `UPDATE recruitment_jobs SET title=?, position_id=?, description=?, requirements=?, closing_date=?, status=? WHERE id=?`,
      [title, posId || old[0].position_id, description || null, requirements || null, closing_date || null, status, req.params.id]
    );
...
export const createApplicant = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [result]: any = await pool.query(
      `INSERT INTO applicants (job_id, first_name, last_name, email, phone, resume_url, cover_letter, status)
       VALUES (?,?,?,?,?,?,?,'new')`,
      [job_id, first_name, last_name, email, phone || null, resume_url || null, cover_letter || null]
    );
...
export const updateApplicantStatus = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [old]: any = await pool.query('SELECT * FROM applicants WHERE id = ?', [req.params.id]);
    if (old.length === 0) return error(res, 'Applicant not found', 404);

    await pool.query(
      `UPDATE applicants SET status=?, interview_date=?, interview_notes=? WHERE id=?`,
      [status, interview_date || null, interview_notes || null, req.params.id]
    );
