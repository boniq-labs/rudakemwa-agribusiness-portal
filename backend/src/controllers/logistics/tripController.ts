...
export const getTrips = async (req: AuthRequest, res: Response) => {
  try {
    ...
    let filters = '';
    if (req.query.status) { filters += ' AND t.status = ?'; params.push(req.query.status); }
    if (req.query.vehicle_id) { filters += ' AND t.vehicle_id = ?'; params.push(req.query.vehicle_id); }
    if (req.query.driver_id) { filters += ' AND t.driver_id = ?'; params.push(req.query.driver_id); }
...
export const createTrip = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [result]: any = await pool.query(
      'INSERT INTO trips (vehicle_id, driver_id, request_id, start_time, end_time, distance_km, fuel_used, destination, purpose, status, trip_number) VALUES (?,?,?,?,?,?,?,?,?,?,?)',
      [vehicle_id, driver_id, request_id || null, start_date || null, end_date || null, distance || null, fuel_used || null, destination || null, purpose || null, status || 'scheduled', `TRIP-${Date.now()}`]
    );
    if (request_id) {
...
export const updateTrip = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [old]: any = await pool.query('SELECT * FROM trips WHERE id = ?', [req.params.id]);
    if (old.length === 0) return error(res, 'Trip not found', 404);
    await pool.query(
      'UPDATE trips SET vehicle_id=?, driver_id=?, start_time=?, end_time=?, distance_km=?, fuel_used=?, destination=?, purpose=?, status=? WHERE id=?',
      [vehicle_id, driver_id, start_date || null, end_date || null, distance || null, fuel_used || null, destination || null, purpose || null, status || 'scheduled', req.params.id]
    );
...
export const updateTripStatus = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (old.length === 0) return error(res, 'Trip not found', 404);
    await pool.query(
      'UPDATE trips SET status=?, end_time=COALESCE(?, end_time), distance_km=COALESCE(?, distance_km), fuel_used=COALESCE(?, fuel_used) WHERE id=?',
      [status, end_date, distance, fuel_used, req.params.id]
    );
