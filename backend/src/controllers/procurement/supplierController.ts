...
export const getSupplierCategories = async (req: AuthRequest, res: Response) => {
  try {
    const [rows]: any = await pool.query('SELECT * FROM supplier_categories WHERE deleted_at IS NULL');
    return success(res, rows);
  } catch (err: any) { return error(res, err.message); }
};

export const createSupplierCategory = async (req: AuthRequest, res: Response) => {
  try {
    const { name, description } = req.body;
    const [result]: any = await pool.query('INSERT INTO supplier_categories (name, description) VALUES (?,?)', [name, description]);
    return created(res, { id: result.insertId }, 'Category created');
  } catch (err: any) { return error(res, err.message); }
};

export const updateSupplierCategory = async (req: AuthRequest, res: Response) => {
  try {
    const { name, description } = req.body;
...
export const updateSupplier = async (req: AuthRequest, res: Response) => {
  try {
    ...
    await pool.query(
      'UPDATE suppliers SET supplier_name=?, email=?, phone=?, address=?, category_id=?, contact_person=? WHERE id=?',
      [supplier_name, email, phone, address, category_id, contact_person || null, req.params.id]
    );
...
export const rateSupplier = async (req: AuthRequest, res: Response) => {
  try {
    const { product_quality, delivery_time, price, reliability, comments } = req.body;
    await pool.query('INSERT INTO supplier_ratings (supplier_id, product_quality, delivery_time, price, reliability, comments, rated_by) VALUES (?,?,?,?,?,?,?)',
      [req.params.id, product_quality, delivery_time, price, reliability, comments, req.user?.id]);
