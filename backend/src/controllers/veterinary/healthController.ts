...
export const getHealthRecords = async (req: AuthRequest, res: Response) => {
  try {
    ...
    let where = 'WHERE 1=1';
    const params: any[] = [];
    if (animal_id) { where += ' AND hr.animal_id = ?'; params.push(animal_id); }
    if (startDate) { where += ' AND hr.checkup_date >= ?'; params.push(startDate); }
    if (endDate) { where += ' AND hr.checkup_date <= ?'; params.push(endDate); }
...
export const getVaccinationSchedule = async (req: AuthRequest, res: Response) => {
  try {
    const [rows]: any = await pool.query('SELECT * FROM vaccination_schedule ORDER BY due_days');
    return success(res, rows);
  } catch (err: any) { return error(res, err.message); }
};
...
export const createVaccinationSchedule = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [result]: any = await pool.query(
      `INSERT INTO vaccination_schedule (name, description, due_days, notes) VALUES (?,?,?,?)`,
      [name, description, due_days, notes]
    );
...
export const getVaccinationRecords = async (req: AuthRequest, res: Response) => {
  try {
    const { animal_id, due } = req.query;
    let where = 'WHERE 1=1';
    const params: any[] = [];
    if (animal_id) { where += ' AND vr.animal_id = ?'; params.push(animal_id); }
    if (due === 'true') { where += ' AND vr.next_due_date <= CURDATE()'; }
    const [rows]: any = await pool.query(
      `SELECT vr.*, a.name as animal_name, a.tag_number, vs.name as vaccine_name
...
export const createVaccinationRecord = async (req: AuthRequest, res: Response) => {
  try {
...
export const getDueVaccinations = async (req: AuthRequest, res: Response) => {
  try {
    const [rows]: any = await pool.query(
      `SELECT a.id as animal_id, a.name as animal_name, a.tag_number,
...
export const getTreatmentPrescriptions = async (req: AuthRequest, res: Response) => {
  try {
    const [rows]: any = await pool.query(
      `SELECT tp.*, mi.name as medicine_name, mi.dosage_form
...
export const createTreatmentPrescription = async (req: AuthRequest, res: Response) => {
  try {
    const { treatment_id, medicine_item_id, dosage, duration, notes } = req.body;
    const [result]: any = await pool.query(
      `INSERT INTO treatment_prescriptions (treatment_id, medicine_item_id, dosage, duration, notes) VALUES (?,?,?,?,?)`,
      [treatment_id, medicine_item_id, dosage, duration, notes]
    );
