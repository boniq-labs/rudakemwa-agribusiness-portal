...
export const getVeterinaryHealth = async (req: AuthRequest, res: Response) => {
  try {
    const pag = getPagination(req);
    const { where, params } = buildWhereClause(pag.filters, pag.search, []);
    let filters = '';
    if (req.query.animal_id) { filters += ' AND hr.animal_id = ?'; params.push(req.query.animal_id); }
...
export const createVeterinaryHealth = async (req: AuthRequest, res: Response) => {
  try {
    const b = req.body;
    ...
    const [result]: any = await pool.query(
      'INSERT INTO animal_health_records (animal_id, checkup_date, diagnosis, prescription, notes, status) VALUES (?,?,?,?,?,?)',
      [b.animal_id, checkup_date, diagnosis, b.treatment || null, notes, b.status || 'open']
    );
...
export const getVeterinaryHealthById = async (req: AuthRequest, res: Response) => {
  try {
    const [rows]: any = await pool.query(
      'SELECT hr.*, a.tag_number, a.name as animal_name FROM animal_health_records hr JOIN animals a ON hr.animal_id = a.id WHERE hr.id = ? AND hr.deleted_at IS NULL',
      [req.params.id]
    );
    if (rows.length === 0) return error(res, 'Health record not found', 404);
    return success(res, rows[0]);
  } catch (err: any) { return error(res, err.message); }
};

export const updateVeterinaryHealth = async (req: AuthRequest, res: Response) => {
  try {
    const [old]: any = await pool.query('SELECT * FROM animal_health_records WHERE id = ?', [req.params.id]);
    if (old.length === 0) return error(res, 'Health record not found', 404);
    const b = req.body;
    const checkup_date = b.date || b.checkup_date || b.record_date || old[0].checkup_date;
    const diagnosis = b.diagnosis || old[0].diagnosis || '';
    const notes = [b.symptoms ? `Symptoms: ${b.symptoms}` : '', b.notes || ''].filter(Boolean).join('\n') || old[0].notes;
    await pool.query(
      'UPDATE animal_health_records SET animal_id=?, checkup_date=?, diagnosis=?, prescription=?, notes=?, status=? WHERE id=?',
      [b.animal_id || old[0].animal_id, checkup_date, diagnosis, b.treatment || old[0].prescription, notes, b.status || old[0].status, req.params.id]
    );
    return success(res, null, 'Health record updated');
  } catch (err: any) { return error(res, err.message); }
};

export const deleteVeterinaryHealth = async (req: AuthRequest, res: Response) => {
  try {
    const [old]: any = await pool.query('SELECT * FROM animal_health_records WHERE id = ?', [req.params.id]);
    if (old.length === 0) return error(res, 'Health record not found', 404);
...
export const getVaccinationSchedule = async (req: AuthRequest, res: Response) => {
  try {
    const [rows]: any = await pool.query('SELECT * FROM vaccination_schedule ORDER BY vaccine_name');
    return success(res, rows);
  } catch (err: any) { return error(res, err.message); }
};

export const createVaccinationSchedule = async (req: AuthRequest, res: Response) => {
  try {
    const { vaccine_name, description, frequency_days, animal_category_id } = req.body;
    const [result]: any = await pool.query(
      'INSERT INTO vaccination_schedule (vaccine_name, description, frequency_days, animal_category_id) VALUES (?,?,?,?)',
      [vaccine_name, description, frequency_days, animal_category_id]
    );
...
export const getVetVaccinations = async (req: AuthRequest, res: Response) => {
  try {
    const pag = getPagination(req);
    const { where, params } = buildWhereClause(pag.filters, pag.search, []);
    let filters = '';
    if (req.query.animal_id) { filters += ' AND vr.animal_id = ?'; params.push(req.query.animal_id); }
...
export const createVetVaccination = async (req: AuthRequest, res: Response) => {
  try {
    const b = req.body;
...
export const updateVetVaccination = async (req: AuthRequest, res: Response) => {
  try {
    const [old]: any = await pool.query('SELECT * FROM vaccination_records WHERE id = ?', [req.params.id]);
    if (old.length === 0) return error(res, 'Vaccination record not found', 404);
    const b = req.body;
...
export const deleteVetVaccination = async (req: AuthRequest, res: Response) => {
  try {
    const [old]: any = await pool.query('SELECT * FROM vaccination_records WHERE id = ?', [req.params.id]);
    if (old.length === 0) return error(res, 'Vaccination record not found', 404);
...
export const getDueVaccinations = async (req: AuthRequest, res: Response) => {
  try {
    const [rows]: any = await pool.query(
      'SELECT vr.*, a.tag_number, a.name as animal_name FROM vaccination_records vr JOIN animals a ON vr.animal_id = a.id WHERE vr.deleted_at IS NULL AND vr.next_due_date IS NOT NULL AND vr.next_due_date <= DATE_ADD(CURDATE(), INTERVAL 7 DAY) ORDER BY vr.next_due_date'
    );
    return success(res, rows);
  } catch (err: any) { return error(res, err.message); }
};
...
export const createPrescription = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [result]: any = await pool.query(
      'INSERT INTO prescriptions (animal_id, medicine, dosage, duration, frequency, start_date, end_date, veterinarian, notes) VALUES (?,?,?,?,?,?,?,?,?)',
      [b.animal_id, medicine, b.dosage || null, b.duration || null, b.frequency || null, b.start_date || null, b.end_date || null, b.veterinarian || null, b.notes || null]
    );
...
export const updatePrescription = async (req: AuthRequest, res: Response) => {
  try {
    const [old]: any = await pool.query('SELECT * FROM prescriptions WHERE id = ?', [req.params.id]);
    if (old.length === 0) return error(res, 'Prescription not found', 404);
    const b = req.body;
