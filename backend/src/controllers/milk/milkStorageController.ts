...
export const getStorageTanks = async (req: AuthRequest, res: Response) => {
  try {
...
export const createStorageTank = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [result]: any = await pool.query(
      `INSERT INTO ${TABLE} (tank_name, tank_number, capacity_liters, temperature, status) VALUES (?,?,?,?,?)`,
      [tank_name, tank_number, capacity_liters, temperature, status || 'available']
    );
...
export const updateStorageTank = async (req: AuthRequest, res: Response) => {
  try {
    const tank_name = req.body.tank_name || req.body.name || '';
    const tank_number = req.body.tank_number || req.body.code || '';
    const capacity_liters = req.body.capacity_liters || req.body.capacity || null;
    ...
    const [old]: any = await pool.query(`SELECT * FROM ${TABLE} WHERE id = ?`, [req.params.id]);
    if (old.length === 0) return error(res, 'Storage tank not found', 404);

    await pool.query(
      `UPDATE ${TABLE} SET tank_name=?, tank_number=?, capacity_liters=?, temperature=?, status=? WHERE id=?`,
      [tank_name, tank_number, capacity_liters, temperature, status, req.params.id]
    );
...
export const getMilkStorage = async (req: AuthRequest, res: Response) => {
  try {
    const pag = getPagination(req);
    const { where, params } = buildWhereClause(pag.filters, pag.search, []);

    let filters = '';
    if (req.query.tank_id) { filters += ' AND ms.tank_id = ?'; params.push(req.query.tank_id); }
    if (req.query.status) { filters += ' AND ms.status = ?'; params.push(req.query.status); }
...
export const addMilkToStorage = async (req: AuthRequest, res: Response) => {
  try {
    const { tank_id, collection_id, quantity_liters, production_date, expiry_date } = req.body;

    const [tank]: any = await pool.query(`SELECT * FROM ${TABLE} WHERE id = ?`, [tank_id]);
    if (tank.length === 0) return error(res, 'Storage tank not found', 404);

    const [current]: any = await pool.query(
      'SELECT SUM(quantity_liters) as total FROM milk_storage WHERE tank_id = ? AND status = "stored" AND deleted_at IS NULL',
      [tank_id]
    );
...
export const getStorageReport = async (req: AuthRequest, res: Response) => {
  try {
    const [rows]: any = await pool.query(
      `SELECT st.id, st.tank_name, st.tank_number, st.capacity_liters,
