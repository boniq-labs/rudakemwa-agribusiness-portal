...
export const createEquipment = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [result]: any = await pool.query(
      ...
      [name, type || '', type || '', serial_number, purchase_date, 1, condition || 'good', status, location, purchase_price || 0, notes || '']
    );
...
export const updateEquipment = async (req: AuthRequest, res: Response) => {
  try {
    ...
    await pool.query(
      ...
      [name, type || '', type || '', serial_number, purchase_date, 1, condition || 'good', status, location, purchase_price || 0, notes || '', req.params.id]
    );
...
export const createEquipmentMaintenance = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [result]: any = await pool.query(
      ...
      [req.params.id, maintenance_type, description, cost, performed_by, maintenance_date, next_maintenance_date]
    );
