...
export const getSalesInvoices = async (req: AuthRequest, res: Response) => {
  try {
...
export const createSalesInvoice = async (req: AuthRequest, res: Response) => {
  try {
    const b = req.body;
    let customer_id = b.customer_id;
    let order_id = b.order_id;
    if (!customer_id && order_id) {
      const [ord]: any = await pool.query('SELECT customer_id FROM sales_orders WHERE id = ?', [order_id]);
      if (ord.length > 0) customer_id = ord[0].customer_id;
    }
    const items = b.items || [];
    const subtotal = items.length ? items.reduce((sum: number, item: any) => sum + (item.quantity * item.unit_price), 0) : 0;
    ...
    for (const item of items) {
      await pool.query(
        `INSERT INTO sales_invoice_items (invoice_id, product_id, description, quantity, unit_price, total_price) VALUES (?,?,?,?,?,?)`,
...
export const recordCustomerPayment = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [inv]: any = await pool.query('SELECT * FROM sales_invoices WHERE id = ?', [invoice_id]);
    if (inv.length === 0) return error(res, 'Invoice not found', 404);
    const [result]: any = await pool.query(
      `INSERT INTO customer_payments (invoice_id, amount, payment_method, reference_number) VALUES (?,?,?,?)`,
      [invoice_id, amount, payment_method || null, reference_number || null]
    );
